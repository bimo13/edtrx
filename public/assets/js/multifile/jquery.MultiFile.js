...
if (window.jQuery)(function ($) {
		...
		this
			...
			.each(function () {
				...
				if (!MultiFile.list) {
					...
					// BIMO:: add col-sm-12 to MultiFile-list (preview wrapper)
					MultiFile.wrapper.append('<div class="MultiFile-list col-sm-12 paddingless" id="' + MultiFile.wrapID + '_list"></div>');
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					// BIMO:: change preview from img to backgrounded-div
					// BIMO:: add col-sm-2 to MultiFile-label
					// BIMO:: add hidden to MultiFile-title
					var names = $('<div class="col-sm-3 text-center mg-top-30px"/>');
					$.each(files, function (i, file) {
						var v = String(file.name || '' ),
								S = MultiFile.STRING,
								n = S.label || S.file || S.name,
								t = S.title || S.tooltip || S.selected,
								p = '<div class="MultiFile-preview" style="'+ MultiFile.previewCss+'"></div>',
								label =	$(
										(
											'<div class="MultiFile-label col-sm-12 marginless paddingless mg-bottom-5px" title="' + t + '">'+
												'<div class="MultiFile-title hidden">'+ n +'</div>'+
						...
						label.find('div.MultiFile-preview').each(function(){
							...
							oFReader.onload = function (oFREvent) {
								...
								t.style.backgroundImage = 'url('+oFREvent.target.result+')';
							};
						});
						...
						names[names.length] =
							(
								'<div class="MultiFile-title" title="' + MultiFile.STRING.selected + '">'
					...
					//$.each(files, function (i, file) {
						...
						var
							...
							b = $('<a class="MultiFile-remove txt-size-12px text-danger" href="#' + MultiFile.wrapID + '">' + MultiFile.STRING.remove + '</a>')
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		STRING: {
			remove: '<i class="glyphicon glyphicon-trash"></i> Remove Image',
